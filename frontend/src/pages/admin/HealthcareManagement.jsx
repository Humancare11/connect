  ...
  return (
    <div className="hcm-page">
      <div className="dash-section">
        <div className="hcm-eyebrow">Super Admin</div>
        <h2 className="dash-section-title" style={{ marginBottom: 6 }}>Healthcare Management</h2>
        <p className="hcm-sub">
          Manage appointment categories, specialties, and conditions for the future appointment tree API.
        </p>

        <div className="dash-cards" style={{ marginTop: 20, marginBottom: 0 }}>
          {stats.map((stat) => (
            <div className="dash-card" key={stat.label}>
              <div className="dash-card-label">{stat.label}</div>
              <div className="dash-card-value" style={{ color: "#6d28d9" }}>{stat.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="dash-section" style={{ marginTop: 24 }}>
        <div className="hcm-toolbar">
          <h2 className="dash-section-title" style={{ marginBottom: 0 }}>{activeSection?.label}</h2>
        ...
        {message.text && (
          <div
            className={message.type === "error" ? "sa-form-error" : "sa-form-success"}
            style={{ marginTop: 16 }}
          >
            {message.text}
          </div>
        ...
        {loading ? (
          <p className="dash-empty">Loading healthcare data...</p>
        ) : (
          <div className="hcm-grid" style={{ marginTop: 20 }}>
            <div>
              <h3 className="hcm-panel-title">
                {editing ? "Edit" : "Create"} {activeSection?.singular}
              </h3>
              <form className="sa-form" onSubmit={submit}>
                ...
                <div className="sp-form-actions">
                  <button className="sa-create-btn" type="submit" disabled={saving}>
                    {saving ? "Saving..." : editing ? `Update ${activeSection?.singular}` : `+ Create ${activeSection?.singular}`}
                  </button>
                  {editing && (
                    <button className="sp-cancel-btn" type="button" onClick={resetForm} disabled={saving}>
                      Cancel
                    </button>
                  )}
                </div>
            ...
            <div>
              <h3 className="hcm-panel-title">{activeSection?.label} ({rows.length})</h3>
                ...
                <table className="dash-table">
                  ...
                  <tbody>
                    {rows.length ? rows.map((item, index) => (
                      <tr key={item._id} className={index % 2 === 0 ? "" : "alt"}>
                        <td>{index + 1}</td>
                        <td>
                        ...
                        {active === "categories" && <td className="muted">{formatUsd(item.price)}</td>}
                        {active === "specialties" && <td className="muted">{relationName(item.categoryId) || "-"}</td>}
                        {active === "conditions" && <td className="muted">{relationName(item.specialtyId?.categoryId) || "-"}</td>}
                        {active === "conditions" && <td className="muted">{relationName(item.specialtyId) || "-"}</td>}
                        <td>
                        ...
                        <td>
                          <div className="actions-cell">
                            <button className="btn-approve" type="button" onClick={() => editItem(item)} disabled={saving}>Edit</button>
                            <button className="btn-reject" type="button" onClick={() => removeItem(item)} disabled={saving}>Delete</button>
                          </div>
                        </td>
                      </tr>
                    )) : (
