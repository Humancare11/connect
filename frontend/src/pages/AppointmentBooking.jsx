...
export default function Ab() {
  ...
  // Re-sync when navigating to this route again with new state (e.g. clicking
  // a "spec"/"cond" deep link while already on /appointment-booking) - the
  // useState initializer above only runs on first mount, not on subsequent
  // navigations to the same route element.
  useEffect(() => {
    setBrowseTab(
      locationState?.tab === "cond" || locationState?.tab === "spec"
        ? locationState.tab
  ...
  return (
    <>
      ...
      <FAQ
        ...
        description="Everything you need to know about booking online doctor appointments, qualifications, and preparing for your visit."
        // stats={[
        //   "Avg. response in 10 min",
        //   "HIPAA secure & private",
        //   "Available Globally",
        // ]}
        sections={APPOINTMENT_FAQS}
      />
    </>
  );
}
