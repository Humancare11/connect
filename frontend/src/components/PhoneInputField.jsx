...
export function parseValue(val, defaultCode) {
  const fallbackCode =
    defaultCode && defaultCode !== "auto" ? defaultCode : "IN";
  const def =
    COUNTRIES.find((c) => c.code === fallbackCode.toUpperCase()) ||
  ...
  if (matches.length > 0) {
    const matched =
      matches.length === 1
        ? matches[0]
        : matches.find((c) => c.code === "US") || matches[0];
...
export default function PhoneInputField({
  ...
  return (
    <>
      ...
      <div ref={wrapRef} className={`pif-control${active ? " is-active" : ""}`}>
          ...
          >
            ...
            >
              ...
              >
                ...
                {searchInputId && (
                  <label htmlFor={searchInputId} style={VISUALLY_HIDDEN}>
                    Search Country
                  </label>
                )}
                <input
