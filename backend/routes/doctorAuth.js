...
// Fields each wizard step is allowed to write. Mirrors buildStepPayload()
// on the frontend. An explicit allow-list (rather than trusting req.body.data
// wholesale) stops a doctor from smuggling protected fields — e.g.
// approvalStatus, verified, doctorId — into a step-save call.
...
// Array / Mixed-type schema fields need markModified() after .set(), since
// Mongoose doesn't always detect in-place changes to these on its own.
...
// ── PATCH /api/doctor/enrollment/step ─────────────────────────────────────────
// Autosave: persists ONE wizard step's field data immediately (on "Save" or
// on Continue), separate from /enrollment/progress above which only tracks
// *which* step the doctor is on. Upserts the Enrollment record so data shows
// up on the admin dashboard right away, even if the doctor never reaches the
// final submit.
router.patch("/enrollment/step", verifyDoctorToken, async (req, res) => {
    try {
        ...
        let enrollment = await Enrollment.findOne({ doctorId });
        if (!enrollment) {
            // Shouldn't normally happen — register/login already auto-create a
            // minimal Enrollment — but handled defensively in case this route is
            // ever hit first.
            const nameParts = doctor.name.trim().split(/\s+/);
            enrollment = new Enrollment({
                doctorId,
        ...
        // An approved/rejected doctor editing via the wizard shouldn't have
        // their live, admin-approved profile silently overwritten field-by-field
        // on every autosave — that path already goes through POST /enrollment,
        // which stages changes for admin review. Autosave here is only for the
        // initial onboarding draft, so skip writing step data once a decision
        // has already been made.
        if (enrollment.approvalStatus === "approved" || enrollment.approvalStatus === "rejected") {
            return res.status(200).json({
                message: "Profile already decided — use the edit/resubmit flow to change it.",
                enrollment,
            });
        }

        const safeData = pickAllowedStepFields(data, STEP_FIELD_ALLOWLIST[stepNum]);
        enrollment.set(safeData);
        for (const key of Object.keys(safeData)) {
            if (STEP_ARRAY_OR_MIXED_FIELDS.has(key)) enrollment.markModified(key);
        }
        enrollment.updatedAt = new Date();
        await enrollment.save();

        return res.status(200).json({ message: "Step saved", enrollment });
    } catch (err) {
        console.error("saveEnrollmentStep error:", err);
